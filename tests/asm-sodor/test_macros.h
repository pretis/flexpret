...
# Modified for only thread 0 to run the tests. 
...
#ifdef STATS
#define TEST_STATS_BEGIN ALL_SCHEDULES RESET_COUNTERS
#else
#define TEST_STATS_BEGIN ALL_SCHEDULES
#endif

#define ALL_SCHEDULES \
...
#define TEST_STATS_END \
        ...
        READ_CYCLE_COUNT(x3) \
        addi x3, x3, -1; \
        TOHOST_SCHEDULING_FREQ(x26, x4) \
        TOHOST_INST_COUNT(x2, x4) \
        TOHOST_CYCLE_COUNT(x3, x4) \
        NEXT_SCHEDULE
#else
#define TEST_STATS_END NEXT_SCHEDULE
#endif

#define NEXT_SCHEDULE \
        addi x26, x26, 1; \
